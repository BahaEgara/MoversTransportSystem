...
class Driver(flask_login.UserMixin, db.Model):
    """
    ...
    licenseId = db.Column(db.String(20), nullable=False, unique=True)
    ...
    offenders = db.relationship("Offender", back_populates="driver")

    def __repr__(self):
        return (
            f"Driver(driverId = {self.driverId}, firstName = "
    ...
    def activateAccount(self):
        ...
        :return self: Driver - the activated Driver instance.
        """
    ...
    def deactivateAccount(self):
        ...
        :return self: Driver - the deactivated Driver instance.
        """
        if self.isActive:
            self.isActive = False
    ...
    def login(self, details=dict()):
        ...
        flask.session["user_type"] = "driver"
    ...
    def deleteProfileImage(self, folder):
        """
        Deletes driver's profile image.
    ...
    def updateProfileImage(self, image, folder):
        ...
        :return self: Driver - the updated Driver instance.
        """
    ...
    def confirm(self, token, expiration=3600):
        """
        Uses a token to confirm the driver's email address.

        :param token: str - Contains driver's email address within it.
    ...
    def sendPasswordResetEmail(self):
        """
        Send password reset email to the driver.
        """
        token = self.generateConfirmationToken()
        reset_link = url_for(
            "accounts.driver_password_reset",
            token=token,
            _scheme="https",
            _external=True,
        )
    ...
    def confirmPasswordResetToken(token, expiration=3600):
        ...
        try:
            data = serializer.loads(token, max_age=expiration)
            driver = Driver.query.filter_by(emailAddress=data).first()
    ...
    def updatePassword(self, current_password, new_password):
        """
        Updates driver's password.

        :param current_password: str - Driver's current password.
        :param new_password: str - Driver's new password.
