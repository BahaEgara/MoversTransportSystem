...
class UpdateEmailAddressForm(FlaskForm):
    ...
    def validate_newEmailAddress(self, field):
        if Farmer.query.filter_by(emailAddress=field.data).first():
...
class UpdatePhoneNumberForm(FlaskForm):
    ...
    def validate_newPhoneNumber(self, field):
        if Farmer.query.filter_by(phoneNumber=field.data).first():
...
class AssignGoodForm(FlaskForm):
    goodId = SelectField(
        "Select Good", validators=[DataRequired()], coerce=int
    )
    quantity = DecimalField("Quantity", validators=[DataRequired()])
    unitPrice = DecimalField("Unit Price", validators=[DataRequired()])
    submit = SubmitField("Assign Good")
